...
public class CallManager {
    ...
    public synchronized static void removeActiveCall(Call call) {
        synchronized (activeCalls) {
            ...
            while (activeCallsIter.hasNext()) {
                String key = activeCallsIter.next(); if (activeCalls.get(key).equals(call)) {
                    toRemove.add(key);
                }
            }
    ...
    private static class AnswerCallThread extends Thread {
        ...
        public AnswerCallThread(Call call, Call existingCall, boolean video) {
            this.call = call; this.existingCall = existingCall; this.video = video;
        }
    ...
    public static void setMute(Call call, boolean isMute) {
        logger.trace("Set mute to " + isMute); new MuteThread(call, isMute).start();
    ...
    private static class MuteThread extends Thread {
        ...
        public MuteThread(Call call, boolean isMute) {
            this.call = call; this.isMute = isMute;
        }
    ...
    public static void putOnHold(Call call, boolean isOnHold) {
        Iterator<? extends CallPeer> peers = call.getCallPeers(); while (peers.hasNext()) {
    ...
    private static class PutOnHoldCallPeerThread extends Thread {
        ...
        public PutOnHoldCallPeerThread(CallPeer callPeer, boolean isOnHold) {
            this.callPeer = callPeer; this.isOnHold = isOnHold;
        }
    ...
    private static class EnableLocalVideoThread extends Thread {
        ...
        public EnableLocalVideoThread(Call call, boolean enable) {
            this.call = call; this.enable = enable;
        }
            ...
            //        extends Thread
    {
        ...
        private CreateCallThread(ProtocolProviderService protocolProvider, Contact contact, String stringContact, boolean video) {
            this.protocolProvider = protocolProvider; this.contact = contact;
            this.stringContact = stringContact; this.video = video;
        }

        public void run() throws Throwable {
            Contact contact = this.contact; String stringContact = this.stringContact;

            if (ConfigurationUtils.isNormalizePhoneNumber()) {
                if (contact != null) {
                    stringContact = contact.getAddress(); contact = null;
                }
