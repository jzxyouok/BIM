...
public class FrameworkStartLevelImpl implements FrameworkStartLevel {
    ...
    private class Command implements Runnable {
        ...
        public Command(int startLevel, FrameworkListener... listeners) {
            this.startLevel = startLevel; this.listeners = listeners;
        }

        public void run() {
            int startLevel = getStartLevel(); FrameworkImpl framework = getFramework();
            ...
            if (startLevel < this.startLevel) {
                for (int intermediateStartLevel = startLevel + 1; intermediateStartLevel <= this.startLevel; intermediateStartLevel++) {
                    int oldStartLevel = getStartLevel(); int newStartLevel = intermediateStartLevel;
