...
public class ReconnectPluginActivator implements BundleActivator, ServiceListener, NetworkConfigurationChangeListener, RegistrationStateChangeListener {
    ...
    public void start(BundleContext bundleContext) throws Exception {
        try {
            logger.logEntry(); ReconnectPluginActivator.bundleContext = bundleContext;
        }
        finally {
            logger.logExit();
        }
        ...
        ServiceReference[] protocolProviderRefs; try {
            ...
            logger.error("Error while retrieving service refs", ex); return;
        }
    ...
    public void serviceChanged(ServiceEvent serviceEvent) {
        ...
        if (sService instanceof NetworkAddressManagerService) {
            switch (serviceEvent.getType()) {
                case ServiceEvent.REGISTERED: if (this.networkAddressManagerService != null) {
                    break;
                }

                    this.networkAddressManagerService = (NetworkAddressManagerService) sService; networkAddressManagerService.addNetworkConfigurationChangeListener(this); break;
                case ServiceEvent.UNREGISTERING: ((NetworkAddressManagerService) sService).removeNetworkConfigurationChangeListener(this); break;
            }
        ...
        switch (serviceEvent.getType()) {
            case ServiceEvent.REGISTERED: this.handleProviderAdded((ProtocolProviderService) sService); break;

            case ServiceEvent.UNREGISTERING: this.handleProviderRemoved((ProtocolProviderService) sService); break;
        }
    }
    ...
    private void handleProviderRemoved(ProtocolProviderService provider) {
        ...
        autoReconnEnabledProviders.remove(provider); needsReconnection.remove(provider);
    ...
    private void traceCurrentPPState() {
        ...
        logger.trace("unregisteringProviders: " + unregisteringProviders); logger.trace("----");
    }
    ...
    private void reconnect(final ProtocolProviderService pp) {
        ...
        final ReconnectTask task = new ReconnectTask(pp); task.delay = delay;
    ...
    private void onNetworkUp() {
        if (delayedNetworkDown != null) {
            delayedNetworkDown.cancel(); delayedNetworkDown = null;
        }
    }
    ...
    private void onNetworkDown() {
        if (!org.jitsi.util.OSUtils.IS_ANDROID) {
            ...
            if (delayedNetworkDown == null) {
                delayedNetworkDown = new Timer(); delayedNetworkDown.schedule(new TimerTask() {
                    @Override
                    public void run() {
                        notifyNetworkDown();
                    }
