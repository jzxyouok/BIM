...
public class H264Parameters implements Serializable {
    ...
    public H264Parameters(String path) throws IOException {
        ...
        RandomAccessFile sampleFile = new RandomAccessFile(path, "r"); try {
            parse("", sampleFile);
        }
        finally {
            sampleFile.close();
    ...
    private void parse(String path, RandomAccessFile file) throws IOException {
        ...
        while (true) {
            // Read box size
            long size = readUnsignedInt32(file); file.read(buffer);
            // Read box name
    ...
    private void parseStsdBox(RandomAccessFile file) throws IOException {
        ...
        while (true) {
            ...
            file.read(buffer, 0, 3); if (buffer[0] == 'v' && buffer[1] == 'c' && buffer[2] == 'C') {
                break;
            }
        }
        /*
        ...
        discard(file, 7); int spsLength = file.read(); seq_parameter_set_rbsp = new byte[spsLength];
        file.read(seq_parameter_set_rbsp);
        // Read the PPS parameter
        discard(file, 2); int ppsLength = file.read(); pic_parameter_set_rbsp = new byte[ppsLength];
    ...
    public void logParamaters() {
        String msg = "PPS: "; for (byte b : getPps()) {
            msg += String.format("%02X", b) + ",";
        } logger.info(msg);

        msg = "SPS: "; for (byte b : getSps()) {
            msg += String.format("%02X", b) + ",";
        } logger.info(msg);
    }
    ...
    static H264Parameters getStoredParameters(VideoFormat formatUsed) {
        ...
        if (spsAndPps.length != 2) {
            logger.error("Invalid store parameters string: " + storedValue); return null;
        }

        H264Parameters params = new H264Parameters();
        params.seq_parameter_set_rbsp = Base64.decode(spsAndPps[0], Base64.DEFAULT);
        params.pic_parameter_set_rbsp = Base64.decode(spsAndPps[1], Base64.DEFAULT); return params;
    }
