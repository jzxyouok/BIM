...
public class SurfaceStream extends CameraStreamBase {
    ...
    public void start() throws IOException {
        super.start(); run = true; captureThread = new Thread() {
            @Override
            public void run() {
                captureLoop();
            }
    ...
    protected void onInitPreview() throws IOException {
        ...
        SurfaceTexture st = surfaceManager.getSurfaceTexture(); camera.setPreviewTexture(st);
    }
    ...
    private void captureLoop() {
        ...
        while (run) {
            SurfaceTexture st = surfaceManager.getSurfaceTexture(); surfaceManager.awaitNewImage();

            // Renders the preview on main thread
            paintLocalPreview();

            //TODO: use frame rate supplied by format control
            long delay = calcStats(); if (delay < 80) {
                try {
                    long wait = 80 - delay; logger.debug("Delaying frame: " + wait);
                    Thread.sleep(wait);
                }
            ...
            inputSurface.ensureIsCurrentCtx(); surfaceManager.drawImage();
            inputSurface.setPresentationTime(st.getTimestamp()); inputSurface.swapBuffers();
    ...
    private void paintLocalPreview() {
        paintDone = false; OSGiActivity.uiHandler.post(new Runnable() {
            @Override
            public void run() {
                try {
                    ...
                    if (previewCtx == null || !myCtxProvider.textureUpdated) {
                        ...
                        previewCtx.ensureIsCurrentCtx(); surfaceManager.drawImage();
                    ...
                    synchronized (paintLock) {
                        paintDone = true; paintLock.notifyAll();
                    }
                }
            }
        });
    ...
    public void read(Buffer buffer) throws IOException {
        Surface surface = (Surface) buffer.getData(); if (camera == null && surface != null) {
            this.encoderSurface = surface; startImpl();
    ...
    public void stop() throws IOException {
        run = false; if (captureThread != null) {
            try {
        ...
        if (surfaceManager != null) {
            surfaceManager.release(); surfaceManager = null;
        }
