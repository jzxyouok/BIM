...
public class PreviewStream extends CameraStreamBase implements Camera.PreviewCallback {
    ...
    protected void onInitPreview() throws IOException {
        ...
        camera.setPreviewDisplay(previewSurface); camera.setPreviewCallbackWithBuffer(this);
    }
    ...
    public void read(Buffer buffer) throws IOException {
        byte[] data; synchronized (bufferQueue) {
            data = bufferQueue.removeLast();
        }

        int w = format.getSize().width; int h = format.getSize().height;
        int outLen = (w * h * 12) / 8;
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
        if (data == null) {
            logger.error("Null data received on callback, " + " invalid buffer size ?"); return;
        }
    ...
    static void YV12toYUV420Planar(final byte[] input, final byte[] output, final int width, final int height) {
        ...
        int uvStride = (int) Math.ceil((yStride / 2) / 16.0) * 16; int ySize = yStride * height;
        int uvSize = uvStride * height / 2;

        int I420uvStride = (int) (((yStride / 2) / 16.0) * 16); int I420uvSize = width * height / 4;
        ...
        if (uvStridePadding == 0) {
            ...
            logger.warn("Not recommended resolution: " + width + "x" + height); int src = ySize;
            int dst = ySize;
    ...
    public static int calcYV12Size(int width, int height) {
        ...
        float uvStride = (int) Math.ceil((yStride / 2) / 16.0) * 16; float ySize = yStride * height;
