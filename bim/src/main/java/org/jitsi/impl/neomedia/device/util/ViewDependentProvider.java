...
public abstract class ViewDependentProvider<T> {
    ...
    public ViewDependentProvider(Activity activity, ViewGroup container) {
        this.activity = activity; this.container = container;
    }
    ...
    protected void ensureViewDestroyed() {
        if (view != null) {
            final View viewToRemove = view; view = null;

            activity.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    container.removeView(viewToRemove); container.setVisibility(View.GONE);
                }
            });
        }
    }
    ...
    synchronized protected void onObjectCreated(T obj) {
        this.providedObject = obj; this.notifyAll();
    }
    ...
    synchronized public T obtainObject() {
        ensureViewCreated(); if (this.providedObject == null) {
            try {
                logger.info("Waiting for object..." + hashCode()); this.wait(CREATE_TIMEOUT);
    ...
    synchronized protected void releaseObject() {
        ...
        this.providedObject = null; this.notifyAll();
    }
    ...
    synchronized public void waitForObjectRelease() {
        if (providedObject != null) {
            try {
                ...
                this.wait(REMOVAL_TIMEOUT); if (providedObject != null) {
