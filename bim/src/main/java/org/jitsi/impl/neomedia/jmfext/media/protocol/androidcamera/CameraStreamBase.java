...
abstract class CameraStreamBase extends AbstractPushBufferStream<DataSource> {
    ...
    protected void startImpl() throws IOException {
        this.camera = Camera.open(cameraId); Exception error = null; try {
            ...
            Camera.Parameters params = camera.getParameters(); Dimension size = format.getSize();
            params.setPreviewSize(size.width, size.height);
            params.setPreviewFormat(ImageFormat.YV12); camera.setParameters(params);
            ...
            logger.error(e, e); error = e;
        }
        // Close camera on error
        if (error != null && camera != null) {
            camera.reconnect(); camera.release(); camera = null; throw new IOException(error);
        }
    }
    ...
    public void stop() throws IOException {
        ...
        if (camera != null) {
            ...
            CameraUtils.releaseCamera(camera); this.camera = null;
        }
    }
    ...
    protected long calcStats() {
        long current = System.currentTimeMillis(); long delay = (current - last);
        last = System.currentTimeMillis();
        // Measure moving average
        if (logger.isDebugEnabled()) {
            avg[idx] = delay; if (++idx == avg.length) {
                idx = 0;
            } long movAvg = 0; for (long anAvg : avg) {
                movAvg += anAvg;
