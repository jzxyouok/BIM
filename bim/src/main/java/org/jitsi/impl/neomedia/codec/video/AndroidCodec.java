...
abstract class AndroidCodec extends AbstractCodec2 {
    ...
    protected AndroidCodec(String name, Class<? extends Format> formatClass, Format[] supportedOutputFormats, boolean isEncoder) {
        super(name, formatClass, supportedOutputFormats); this.isEncoder = isEncoder;
    }
    ...
    protected void doOpen() throws ResourceUnavailableException {
        ...
        try {
            ...
            throw new ResourceUnavailableException("Could not create codec for name: " + codecInfo.getName());
        }
        ...
        codecInputs = codec.getInputBuffers(); codecOutputs = codec.getOutputBuffers();
    ...
    protected void doClose() {
        if (codec != null) {
            codec.stop(); codec.release(); codec = null; codecInputs = null; codecOutputs = null;
        }
    }
    ...
    protected int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
        try {
            ...
            logger.error(e, e); throw new RuntimeException(e);
        }
    }
    ...
    private int doProcessImpl(Buffer inputBuffer, Buffer outputBuffer) {
        ...
        if (mediaCodecOutputIndex == MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED) {
            ...
            logger.info("Output format changed to: " + outFormat); if (!isEncoder) {
            ...
            try {
                if (!isEncoder && useSurface()) {
                    processed &= ~OUTPUT_BUFFER_NOT_FILLED; outputBuffer.setFormat(outputFormat);
                }
                else if ((outputLength = info.size) > 0) {
                    byteBuffer = codecOutputs[mediaCodecOutputIndex];
                    byteBuffer.position(info.offset); byteBuffer.limit(info.offset + info.size);

                    byte[] out = AbstractCodec2.validateByteArraySize(outputBuffer, info.size, false);
                    byteBuffer.get(out, 0, info.size);

                    outputBuffer.setFormat(outputFormat); outputBuffer.setLength(outputLength);
        ...
        if (isEncoder && useSurface()) {
            inputBuffer.setData(getSurface()); processed &= ~INPUT_BUFFER_NOT_CONSUMED;
        }
        else {
            int mediaCodecInputIndex = codec.dequeueInputBuffer(0); if (mediaCodecInputIndex >= 0) {
                ...
                codecInput.clear(); codecInput.put(input, inputOffset, inputLen);
                codec.queueInputBuffer(mediaCodecInputIndex, 0, inputLen,
                        /* presentationTimeUs */ inputBuffer.getTimeStamp(),
                        /* flags */ 0);

                processed &= ~INPUT_BUFFER_NOT_CONSUMED; if (logger.isTraceEnabled()) {
