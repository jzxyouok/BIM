...
public abstract class CodecInfo {
    ...
    static {
        ...
        // Crashes
        bannedYuvCodecs.add("OMX.SEC.avc.enc"); bannedYuvCodecs.add("OMX.SEC.h263.enc");
        ...
        for (int codecIndex = 0, codecCount = MediaCodecList.getCodecCount(); codecIndex < codecCount; codecIndex++) {
            ...
            CodecInfo ci = CodecInfo.getCodecInfo(codecInfo); if (ci != null) {
                codecs.add(ci); ci.setBanned(bannedYuvCodecs.contains(ci.getName()));
            }
    ...
    public CodecInfo(MediaCodecInfo codecInfo, String mediaType) {
        this.codecInfo = codecInfo; this.mediaType = mediaType;
        this.caps = codecInfo.getCapabilitiesForType(mediaType);

        this.colors = new ArrayList<CodecColorFormat>(); int[] colorFormats = caps.colorFormats;
    ...
    public ProfileLevel[] getProfileLevels() {
        if (profileLevels == null) {
            ...
            for (int i = 0; i < profileLevels.length; i++) {
                Profile p = getProfile(plArray[i].profile); Level l = getLevel(plArray[i].level);
    ...
    public String toString() {
        ...
        for (int i = 0; i < colors.size(); i++) {
            colorStr.append(colors.get(i)); if (i != colors.size() - 1) {
                colorStr.append(", \n");
            }
        }

        StringBuilder plStr = new StringBuilder("\nprofiles:\n");
        ProfileLevel[] profiles = getProfileLevels(); for (int i = 0; i < profiles.length; i++) {
            plStr.append(profiles[i].toString()); if (i != profiles.length - 1) {
                plStr.append(", \n");
            }
        }
    ...
    public static CodecInfo getCodecInfo(MediaCodecInfo codecInfo) {
        String[] types = codecInfo.getSupportedTypes(); for (String type : types) {
            try {
