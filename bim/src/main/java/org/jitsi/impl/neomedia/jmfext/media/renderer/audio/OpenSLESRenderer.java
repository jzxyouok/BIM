...
public class OpenSLESRenderer extends AbstractAudioRenderer<AudioSystem> {
    ...
    public synchronized void close() {
        if (ptr != 0) {
            close(ptr); ptr = 0; setThreadPriority = true;
        }
    }
    ...
    public synchronized void open() throws ResourceUnavailableException {
        if (ptr == 0) {
            AudioFormat inputFormat = this.inputFormat; int channels = inputFormat.getChannels();
    ...
    public synchronized void start() {
        if (ptr != 0) {
            setThreadPriority = true; start(ptr);
        }
    }
    ...
    public synchronized void stop() {
        if (ptr != 0) {
            stop(ptr); setThreadPriority = true;
        }
    }
