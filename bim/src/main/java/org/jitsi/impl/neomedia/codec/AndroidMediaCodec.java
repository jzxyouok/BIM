...
public class AndroidMediaCodec extends AbstractCodec2 {
        ...
        for (int codecIndex = 0, codecCount = MediaCodecList.getCodecCount(); codecIndex < codecCount; codecIndex++) {
            ...
            for (String supportedType : supportedTypes) {
                ...
                if (codecInfo.isEncoder()) {
                     ...
                     */
                    a = supportedOutputFormats; b = supportedInputFormats;
                }
                else {
                    /*
                     * Android's supportedType i.e. aSupportedFormat specifies
                     ...
                     */
                    a = supportedInputFormats; b = supportedOutputFormats;
    ...
    private static VideoFormat getFmjFormatFromMediaCodecColorFormat(int colorFormat) {
        ...
        for (int i = 0; i < PIX_FMTS_TO_MEDIA_CODEC_COLOR_FORMATS.length; i += 2) {
            if (PIX_FMTS_TO_MEDIA_CODEC_COLOR_FORMATS[i + 1] == colorFormat) {
                pixfmt = PIX_FMTS_TO_MEDIA_CODEC_COLOR_FORMATS[i]; break;
            }
        }
    ...
    private static Format getFmjFormatFromMediaCodecType(String type) {
        ...
        for (int i = 0; i < FMJ_ENCODINGS_TO_MEDIA_CODEC_TYPES.length; i += 2) {
            if (FMJ_ENCODINGS_TO_MEDIA_CODEC_TYPES[i + 1].equals(type)) {
                encoding = FMJ_ENCODINGS_TO_MEDIA_CODEC_TYPES[i]; break;
            }
        }
    ...
    protected void doClose() {
        if (mediaCodec != null) {
            try {
                 ...
                 */
                colorFormat = OMX_COLOR_FormatUnused; inputBuffers = null; outputBuffers = null;
                type = null;
            }
        }

        if (avFrame != null) {
            avFrame.free(); avFrame = null;
    ...
    protected int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
        ...
        if (MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED == mediaCodecOutputIndex) {
            ...
            int outputLength = 0; java.nio.ByteBuffer byteBuffer = null;
                ...
                if ((outputLength = info.size) > 0) {
                    ...
                    if ((avFrameData == null) || (avFrameData.getCapacity() < outputLength)) {
                        ...
                        for (int i = info.offset, end = i + outputLength; i < end; i++) {
                            b[0] = byteBuffer.get(i); FFmpeg.memcpy(ptr, b, 0, b.length); ptr++;
                        }

                        outputBuffer.setData(avFrame); outputBuffer.setFormat(outputFormat);
                        outputBuffer.setLength(outputLength); outputBuffer.setOffset(0);
        ...
        if (0 <= mediaCodecInputIndex) {
            int mediaCodecInputOffset = 0; int mediaCodecInputLength = 0;
    ...
    private void maybeConfigureAndStart() {
        ...
        if (mediaCodec != null) {
            ...
            if (encoder) {
                typeFormat = outputFormat; colorFormatFormat = inputFormat;
            }
            else {
                typeFormat = inputFormat; colorFormatFormat = outputFormat;
            }
            ...
            for (String supportedType : supportedTypes) {
                ...
                for (int colorFormat : colorFormats) {
                    if (matchesMediaCodecColorFormat(colorFormatFormat, colorFormat)) {
                         ...
                         */
                        this.colorFormat = colorFormat; this.type = supportedType; break;
                    }
                }
                /*
         ...
         */
        if ((this.colorFormat != OMX_COLOR_FormatUnused) && (this.type != null)) {
            MediaCodec mediaCodec = null; try {
            ...
            if (mediaCodec != null) {
                MediaFormat format = new MediaFormat(); int flags = 0;

                format.setString(MediaFormat.KEY_MIME, this.type); if (codecInfo.isEncoder()) {
                    encoder = true; flags |= MediaCodec.CONFIGURE_FLAG_ENCODE;
                ...
                mediaCodec.configure(format, null, null, flags); mediaCodec.start();

                this.mediaCodec = mediaCodec; inputBuffers = mediaCodec.getInputBuffers();
                outputBuffers = mediaCodec.getOutputBuffers();
    ...
    public Format setInputFormat(Format format) {
        Format oldValue = inputFormat; Format setInputFormat = super.setInputFormat(format);
    ...
    public Format setOutputFormat(Format format) {
        ...
        Format oldValue = outputFormat; Format setOutputFormat = super.setOutputFormat(format);
