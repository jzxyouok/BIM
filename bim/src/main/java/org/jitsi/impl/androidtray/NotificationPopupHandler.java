...
public class NotificationPopupHandler extends AbstractPopupMessageHandler implements ChatSessionManager.CurrentChatListener {
    ...
    public void showPopupMessage(PopupMessage popupMessage) {
        ...
        for (AndroidPopup popup : notificationMap.values()) {
            AndroidPopup merge = popup.tryMerge(popupMessage); if (merge != null) {
                newPopup = merge; break;
            }
        }
        ...
        JitsiApplication.getNotificationManager().notify(nId, builder.build()); newPopup.onPost();
    ...
    void fireNotificationClicked(int notificationId) {
        ...
        AndroidPopup popup = notificationMap.get(notificationId); if (popup == null) {
            logger.error("No valid notification exists for " + notificationId); return;
        }

        PopupMessage msg = popup.getPopupMessage(); if (msg == null) {
            logger.error("No popup message found for " + notificationId); return;
        }
    ...
    private void removeNotification(int notificationId) {
        ...
        AndroidPopup popup = notificationMap.get(notificationId); if (popup == null) {
            logger.warn("Notification for id: " + notificationId + " already removed"); return;
        }

        logger.debug("Removing notification: " + notificationId); popup.removeNotification();
