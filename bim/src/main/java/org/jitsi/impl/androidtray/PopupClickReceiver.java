...
public class PopupClickReceiver extends BroadcastReceiver {
    ...
    void registerReceiver() {
        IntentFilter filter = new IntentFilter(); filter.addAction(POPUP_CLICK_ACTION);
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (notificationId == -1) {
            logger.warn("Invalid notification id = -1"); return;
        }

        String action = intent.getAction(); if (action.equals(POPUP_CLICK_ACTION)) {
    ...
    public static Intent createIntent(int notificationId) {
        Intent intent = new Intent(); intent.setAction(POPUP_CLICK_ACTION);
        intent.putExtra(EXTRA_NOTIFICATION_ID, notificationId); return intent;
    }
    ...
    public static Intent createDeleteIntent(int notificationId) {
        Intent intent = new Intent(); intent.setAction(POPUP_CLEAR_ACTION);
        intent.putExtra(EXTRA_NOTIFICATION_ID, notificationId); return intent;
    }
}
