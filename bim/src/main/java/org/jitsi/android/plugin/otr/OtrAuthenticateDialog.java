...
public class OtrAuthenticateDialog extends OSGiActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String localFingerprint = OtrActivator.scOtrKeyManager.getLocalFingerprint(contact.getProtocolProvider().getAccountID());

        View content = findViewById(android.R.id.content);
        ViewUtil.setTextViewValue(content, R.id.localFingerprintLbl, OtrActivator.resourceService.getI18NString("plugin.otr.authbuddydialog.LOCAL_FINGERPRINT", new String[]{account, localFingerprint}));

        // Remote fingerprint.
        String user = contact.getDisplayName();
        String remoteFingerprint = OtrActivator.scOtrKeyManager.getRemoteFingerprint(contact);

        ViewUtil.setTextViewValue(content, R.id.remoteFingerprintLbl, OtrActivator.resourceService.getI18NString("plugin.otr.authbuddydialog.REMOTE_FINGERPRINT", new String[]{user, remoteFingerprint}));

        // Action
        ViewUtil.setTextViewValue(content, R.id.actionTextView, OtrActivator.resourceService.getI18NString("plugin.otr.authbuddydialog.VERIFY_ACTION", new String[]{user}));

        // Verify button
        ViewUtil.setCompoundChecked(getContentView(), R.id.verifyButton, OtrActivator.scOtrKeyManager.isVerified(contact));
    }
    ...
    public static Intent createIntent(UUID uuid) {
        Intent intent = new Intent(JitsiApplication.getGlobalContext(), OtrAuthenticateDialog.class);
