...
public class VideoCallActivity extends OSGiActivity implements CallPeerRenderer, CallRenderer, CallChangeListener, PropertyChangeListener, ZrtpInfoDialog.SasVerificationListener, AutoHideController.AutoHideListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (call == null) {
            logger.error("There's no call with id: " + callIdentifier); return;
        }
        ...
        View.OnClickListener toastclickHandler = new View.OnClickListener() {
            public void onClick(View v) {
                showZrtpInfoDialog(); sasToastController.hideToast(true);
            }
        };
        ...
        if (savedInstanceState == null) {
            VideoHandlerFragment videoFragment; if (AndroidUtils.hasAPI(18)) {
    ...
    protected void onRestoreInstanceState(Bundle savedInstanceState) {
        super.onRestoreInstanceState(savedInstanceState); if (sasToastController != null) {
    ...
    private void initMicrophoneView() {
        ...
        microphoneButton.setOnLongClickListener(new View.OnLongClickListener() {
            public boolean onLongClick(View view) {
                ...
                newFragment.show(getSupportFragmentManager(), "vol_ctrl_dialog"); return true;
            }
        });
    }
    ...
    private void initSpeakerphoneButton() {
        ...
        speakerphoneButton.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                onCallVolumeClicked(v); return true;
            }
        });
    }
    ...
    protected void onResume() {
        ...
        Iterator<? extends CallPeer> peers = call.getCallPeers(); if (peers.hasNext()) {
            CallPeer callPeer = peers.next(); addCallPeerUI(callPeer);
        }
        else {
            if (!callState.callEnded) {
                logger.error("There aren't any peers in the call"); finish();
            } return;
        }

        doUpdateHoldStatus(); doUpdateMuteStatus(); updateSpeakerphoneStatus();
        initSecurityStatus();
    }
    ...
    protected void onPause() {
        ...
        if (callPeerAdapter != null) {
            ...
            } callPeerAdapter.dispose(); callPeerAdapter = null;
        }
    ...
    public void setPeerName(final String name) {
        ...
        runOnUiThread(new Runnable() {
            public void run() {
                actionBar.setTitle(getResources().getString(R.string.service_gui_CALL_WITH) + ": " + name);
    ...
    public void setErrorReason(final String reason) {
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                if (errorReason != null) {
                    errorReason.setText(reason); errorReason.setVisibility(View.VISIBLE);
                }
            }
        });
    }
    ...
    private boolean isOnHold() {
        boolean onHold = false; Iterator<? extends CallPeer> peers = call.getCallPeers();
        if (peers.hasNext()) {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        MenuInflater inflater = getMenuInflater(); inflater.inflate(R.menu.video_call_menu, menu);
        return true;
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (itemId == R.id.low_resolution) {
            ...
            showCallInfoDialog(); return true;
        }
        else if (itemId == R.id.call_zrtp_info_item) {
            showZrtpInfoDialog(); return true;
        }
        else {
    ...
    private void onCallConferenceEventObject(EventObject ev) {
        ...
        try {
            ...
            if (tryStopCallTimer && (callConference.isEnded() || callConference.getCallPeerCount() == 0)) {
                stopCallTimer(); doFinishActivity();
            }
        }
        finally {
    ...
    private void initSecurityStatus() {
        boolean isSecure = false; boolean isVerified = false; ZrtpControl zrtpCtrl = null;

        Iterator<? extends CallPeer> callPeers = call.getCallPeers(); if (callPeers.hasNext()) {
            ...
            if (cpCandidate instanceof MediaAwareCallPeer<?, ?, ?>) {
                ...
                if (srtpCtrl instanceof ZrtpControl) {
                    zrtpCtrl = (ZrtpControl) srtpCtrl; isVerified = zrtpCtrl.isSecurityVerified();
                }
                else {
                    isVerified = true;
                }
    ...
    private void doUpdatePadlockStatus(boolean isSecure, boolean isVerified) {
        if (isSecure) {
            if (isVerified) {
                // Security on
                setPadlockColor(R.color.green_padlock); setPadlockSecure(true);
            }
            else {
                // Security pending
                setPadlockColor(R.color.orange_padlock); setPadlockSecure(true);
            }
        }
        else {
            // Security off
            setPadlockColor(R.color.red_padlock); setPadlockSecure(false);
        }
    }
    ...
    private void setPadlockColor(int colorId) {
        ...
        int color = getResources().getColor(colorId); padlockGroup.setBackgroundColor(color);
    }
    ...
    public void securityOn(final CallPeerSecurityOnEvent evt) {
        runOnUiThread(new Runnable() {
            public void run() {
                SrtpControl srtpCtrl = evt.getSecurityController(); ZrtpControl zrtpControl = null;
                if (srtpCtrl instanceof ZrtpControl) {
