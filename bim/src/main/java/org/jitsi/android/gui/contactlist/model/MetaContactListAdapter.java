...
public class MetaContactListAdapter extends BaseContactListAdapter implements MetaContactListListener, ContactPresenceStatusListener, UIGroupRenderer {
    ...
    private void addContacts(final MetaContactGroup group) {
        ...
        Iterator<MetaContactGroup> subGroups = group.getSubgroups(); while (subGroups.hasNext()) {
    ...
    private void removeGroup(final MetaContactGroup metaGroup) {
        ...
        if (origGroupIndex >= 0) {
            originalContacts.remove(origGroupIndex); originalGroups.remove(metaGroup);
        }

        int groupIndex = groups.indexOf(metaGroup);

        if (groupIndex >= 0) {
            contacts.remove(groupIndex); groups.remove(metaGroup);
        }
    }
    ...
    private void removeContact(final MetaContactGroup metaGroup, final MetaContact metaContact) {
        ...
        int groupIndex = groups.indexOf(metaGroup); if (groupIndex >= 0) {
            TreeSet<MetaContact> contactList = getContactList(groupIndex);
    ...
    public void protoContactMoved(final ProtoContactEvent evt) {
        uiHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                updateStatus(evt.getOldParent()); updateStatus(evt.getNewParent());
            }
        });
    }
    ...
    public void metaContactMoved(final MetaContactMovedEvent evt) {
        uiHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                oldGroupIdx = groups.indexOf(oldParent); newGroupIdx = groups.indexOf(newParent);
                if (oldGroupIdx < 0 || newGroupIdx < 0) {
    ...
    public Object getChild(int groupPosition, int childPosition) {
        ...
        Iterator<MetaContact> contactList = getContactList(groupPosition).iterator(); int i = 0;
        while (contactList.hasNext()) {
            MetaContact metaContact = contactList.next(); if (i == childPosition) {
                return metaContact;
            } i++;
        }
    ...
    private int getChildIndex(TreeSet<MetaContact> contactList, MetaContact metaContact) {
        ...
        int i = 0; while (contactListIter.hasNext()) {
    ...
    public void filterData(final String query) {
        ...
        groups.clear(); contacts.clear();

        if (query.isEmpty()) {
            groups.addAll(originalGroups); contacts.addAll(originalContacts);
        }
        else {
            for (MetaContactGroup metaGroup : originalGroups) {
                ...
                if (filteredList.size() > 0) {
                    groups.add(metaGroup); contacts.add(filteredList);
                }
            }
        }
    ...
    private boolean isMatching(MetaContact metaContact, String query) {
        ...
        if (queryPattern.matcher(metaContact.getDisplayName()).find()) {
            ...
            Iterator<Contact> contacts = metaContact.getContacts(); while (contacts.hasNext()) {
