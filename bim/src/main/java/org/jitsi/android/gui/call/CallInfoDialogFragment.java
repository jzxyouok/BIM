...
public class CallInfoDialogFragment extends OSGiDialogFragment {
    ...
    public static CallInfoDialogFragment newInstance(String callKey) {
        ...
        Bundle args = new Bundle(); args.putString(CALL_KEY_EXTRA, callKey); f.setArguments(args);
    ...
    private void doUpdateView() {
        CallConference conference = call.getConference(); List<Call> calls = conference.getCalls();
        if (calls.size() == 0) {
            return;
        }
        ...
        List<CallPeer> callPeers = conference.getCallPeers(); if (callPeers.size() == 0) {
            return;
        }
    ...
    private void updateIceSection(CallPeerMediaHandler<?> callPeerMediaHandler) {
        // ICE state.
        String iceState = null; if (callPeerMediaHandler != null) {
        ...
        long harvestingTime = 0; if (callPeerMediaHandler != null) {
    ...
    private String getHarvestTimeStr(long harvestingTime, int harvestCount) {
        ...
        harvestStr.append(harvestingTime); harvestStr.append(" ");
        harvestStr.append(getString(R.string.service_gui_callinfo_HARVESTING_MS_FOR));
        harvestStr.append(" "); harvestStr.append(harvestCount);
    ...
    private String getStreamEncryptionMethod(CallPeerMediaHandler<?> callPeerMediaHandler, MediaStream mediaStream, MediaType mediaType) {
        ...
        if (srtpControl != null) {
            String info; if (srtpControl instanceof ZrtpControl) {
    ...
    private void updateAudioVideoInfo(CallPeerMediaHandler<?> callPeerMediaHandler, MediaType mediaType) {
        ...
        MediaStreamStats mediaStreamStats = null; if (mediaStream != null) {
        ...
        ensureVisible(container, R.id.remoteIpLabel, !displayedIpPort); if (!displayedIpPort) {
    ...
    private void startUpdateThread() {
        this.pollingThread = new InfoUpdateThread(); pollingThread.start();
    }
    ...
    private void stopUpdateThread() {
        if (pollingThread != null) {
            pollingThread.ensureFinished(); pollingThread = null;
        }
    }
