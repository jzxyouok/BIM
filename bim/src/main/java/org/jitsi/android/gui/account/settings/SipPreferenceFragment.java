...
public class SipPreferenceFragment extends AccountPreferenceFragment {
    ...
    protected void onInitPreferences() {
        ...
        String password = registration.getPassword(); editor.putString(PREF_KEY_USER_ID, userId);
        ...
        int cListTypeIdx = 0; if (registration.isXCapEnable()) {
            cListTypeIdx = 1;
        }
    ...
    protected void onPreferencesCreated() {
        ...
        String[] entries = new String[certs.size()]; entries = certs.toArray(entries);
        ListPreference certPreference = (ListPreference) findPreference(PREF_KEY_TLS_CERT_ID);
        certPreference.setEntries(entries); certPreference.setEntryValues(entries);
