...
public class VideoHandlerFragment extends OSGiFragment {
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        ctrlButtonsGroup.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                // We know the size of all components at this point, so we can init layout dependent stuff
    ...
    public void onResume() {
        ...
        if (call == null) {
            logger.error("Call is null"); return;
        }
        ...
        Iterator<? extends CallPeer> peers = call.getCallPeers(); if (peers.hasNext()) {
            CallPeer callPeer = peers.next(); addVideoListener(callPeer); initRemoteVideo(callPeer);
        }
        else {
    ...
    public void onPause() {
        ...
        if (call.getCallState() != CallState.CALL_ENDED) {
             ...
             */
            setLocalVideoEnabled(false); previewSurfaceHandler.waitForObjectRelease();
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        inflater.inflate(R.menu.camera_menu, menu); this.menu = menu;
    ...
    public void onLocalVideoButtonClicked(View callVideoButton) {
        ...
        if (isEnable) {
            ...
            callVideoButton.setBackgroundColor(android.graphics.Color.TRANSPARENT);
        }
    }
    ...
    private void setLocalVideoEnabled(boolean enable) {
        if (call == null) {
            ...
            logger.error("调用摄像头实例为空(调用已经结束了吗?)"); return;
        }
    ...
    public void handleVideoEvent(CallPeer callPeer, final VideoEvent event) {
        ...
        {
            // TODO: local video events are not used because the preview surface is required for camera to start and it must not be removed until is stopped,
            // so it's handled by direct cooperation with jmfext.media.protocol.mediarecorder.DataSource
    ...
    private void handleRemoteVideoEvent(final Component visualComponent, final SizeChangeVideoEvent scve) {
        if (visualComponent instanceof ViewAccessor) {
            ...
            logger.trace("Remote video removed " + hashCode()); this.remoteVideoAccessor = null;
    ...
    private Dimension selectRemotePreferredSize(Component visualComponent, View remoteVideoView, SizeChangeVideoEvent scve) {
        ...
        if ((preferredSize != null) && (preferredSize.width > 0) && (preferredSize.height > 0)) {
            /*
             * If the visualComponent displaying the video of the remote callPeer has a preferredSize, attempt to respect it.
             */
            ...
            if ((scve.getHeight() > 0) && (scve.getWidth() > 0)) {
                height = scve.getHeight(); width = scve.getWidth();
            }
        }
    ...
    private void doAlignRemoteVideo(View remoteVideoView, Dimension preferredSize) {
        if (remoteVideoView != null) {
            ...
            params.addRule(RelativeLayout.CENTER_VERTICAL); callInfoGroup.setLayoutParams(params);
    ...
    void updateCallInfoMargin() {
        ...
        if (remoteVideoContainer.getChildCount() > 0) {
            ...
            params.setMargins(0, 0, 0, marginBottom); callInfoGroup.setLayoutParams(params);
        }
        else {
            params.setMargins(0, 0, 0, 0); callInfoGroup.setLayoutParams(params);
        }
    }
}
