...
public class DialogActivity extends OSGiActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.alert_dialog); View content = findViewById(android.R.id.content);
        ...
        // Confirm button text
        String confirmTxt = intent.getStringExtra(EXTRA_CONFRIM_TXT); if (confirmTxt != null) {
        ...
        // Sets the listener
        this.listenerID = intent.getLongExtra(EXTRA_LISTENER_ID, -1); if (listenerID != -1) {
        ...
        long dialogId = intent.getLongExtra(EXTRA_DIALOG_ID, -1); if (dialogId != -1) {
            ...
            synchronized (displayedDialogs) {
                displayedDialogs.add(dialogId); displayedDialogs.notifyAll();
            }
        }
    }
    ...
    protected void onDestroy() {
        ...
        if (closeIntentListener != null) {
            ...
            synchronized (displayedDialogs) {
                displayedDialogs.remove(listenerID); displayedDialogs.notifyAll();
            }
        }
    ...
    public static void closeDialog(Context ctx, long dialogId) {
        Intent intent = new Intent(ACTION_CLOSE_DIALOG); intent.putExtra(EXTRA_DIALOG_ID, dialogId);
    ...
    public static void showDialog(Context ctx, String title, String message) {
        Intent alert = getDialogIntent(ctx, title, message); ctx.startActivity(alert);
    }
    ...
    public static Intent getDialogIntent(Context ctx, String title, String message) {
        Intent alert = new Intent(ctx, DialogActivity.class); alert.putExtra(EXTRA_TITLE, title);
        alert.putExtra(EXTRA_MESSAGE, message); alert.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        return alert;
    }
    ...
    public static void showConfirmDialog(Context context, String title, String message, String confirmTxt, DialogListener listener) {
        ...
        if (listener != null) {
            long listenerID = System.currentTimeMillis(); listenersMap.put(listenerID, listener);
            alert.putExtra(EXTRA_LISTENER_ID, listenerID);
        }

        alert.putExtra(EXTRA_TITLE, title); alert.putExtra(EXTRA_MESSAGE, message);
    ...
    public static long showCustomDialog(Context context, String title, String fragmentClass, Bundle fragmentArguments, String confirmTxt, DialogListener listener, Map<String, Serializable> extraArguments) {
        ...
        if (listener != null) {
            listenersMap.put(dialogId, listener); alert.putExtra(EXTRA_LISTENER_ID, dialogId);
        }

        alert.putExtra(EXTRA_TITLE, title); alert.putExtra(EXTRA_CONFRIM_TXT, confirmTxt);
    ...
    public static boolean waitForDialogOpened(long dialogId) {
        synchronized (displayedDialogs) {
            if (!displayedDialogs.contains(dialogId)) {
                try {
                    displayedDialogs.wait(10000); return displayedDialogs.contains(dialogId);
                }
