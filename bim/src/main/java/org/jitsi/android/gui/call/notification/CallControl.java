...
public class CallControl extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        String callID = intent.getStringExtra(EXTRA_CALL_ID); if (callID == null) {
            logger.error("Extra call ID is null"); return;
        }

        Call call = CallManager.getActiveCall(callID); if (call == null) {
            logger.error("Call with id: " + callID + " does not exists"); return;
        }

        int action = intent.getIntExtra(EXTRA_ACTION, -1); if (action == -1) {
            logger.error("No action supplied"); return;
        }

        if (action == ACTION_HANGUP) {
            logger.trace("Action HANGUP"); CallManager.hangupCall(call);
        }
        else if (action == ACTION_TOGGLE_MUTE) {
            logger.trace("Action TOGGLE MUTE"); boolean isMute = CallManager.isMute(call);
