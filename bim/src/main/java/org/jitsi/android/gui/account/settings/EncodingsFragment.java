...
public class EncodingsFragment extends Fragment implements TouchInterceptor.DropListener {
    ...
    public void setEnabled(boolean isEnabled) {
        this.isEnabled = isEnabled; adapter.invalidate();
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        listWidget.setAdapter(adapter); listWidget.setDropListener(this); return content;
    }
    ...
    public void drop(int from, int to) {
        adapter.swapItems(from, to); hasChanges = true;
    }
    ...
    static public EncodingsFragment newInstance(List<String> encodings, List<Integer> priorities) {
        ...
        Bundle args = new Bundle(); args.putSerializable(ARG_ENCODINGS, (Serializable) encodings);
    ...
    class OrderListAdapter extends BaseAdapter {
        ...
        void swapItems(int from, int to) {
            // Swap positions
            String swap = encodings.get(from); int swapPrior = priorities.get(from);
            encodings.remove(from); priorities.remove(from);

            // Swap priorities
            encodings.add(to, swap); priorities.add(to, swapPrior);
        ...
        public View getView(final int i, View view, ViewGroup viewGroup) {
            ...
            cb.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                public void onCheckedChanged(CompoundButton cButton, boolean b) {
                    priorities.set(i, b ? calcPriority(i) : 0); hasChanges = true;
                }
            });
            // Create string for given format entry
            String mf = encodings.get(i);
            TextView tv = (TextView) gv.findViewById(android.R.id.text1); tv.setText(mf);
            // Creates the drag handle view(used to grab list entries)
            ImageView iv = (ImageView) gv.findViewById(R.id.dragHandle); if (!isEnabled) {
                gv.removeView(iv);
            } cb.setEnabled(isEnabled); tv.setEnabled(isEnabled);
